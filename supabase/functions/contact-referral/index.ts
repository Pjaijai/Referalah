...
serve(async (req: any) => {
  try {
    ...
    const { data: receiver } = await client.from("user").select("*").eq("uuid", to_uuid).single();
    ...
    const subject = `${sender.username} 想搵你做${type === "referer" ? "推薦人" : "受薦人"}`;
    const body = `
      <html lang="zh-Hk">
      <body>
    ...
    const { error: insertError } = await client.from("referral_contact_history").insert({
